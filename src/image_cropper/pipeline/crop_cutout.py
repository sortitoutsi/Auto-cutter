#!/usr/bin/env python3
"""
Portrait cropper: crops images from top-of-head down through the shirt neckline,
horizontally centered so both ears have equal margins to the canvas edge.
Output: 250x250 PNG.

Only processes images that have enough pixels below the chin to include a shirt
area — images cropped tight to the chin/neck are skipped.

Usage:
  python -m image_cropper.pipeline.crop_cutout                    # input/ → output/portrait/
  python -m image_cropper.pipeline.crop_cutout src/ out/          # custom dirs
  python -m image_cropper.pipeline.crop_cutout image.png out/     # single file
"""

from __future__ import annotations

import argparse
import contextlib
import sys
from pathlib import Path

import numpy as np
from PIL import Image

from image_cropper.errors import DetectionError, ImageCropperError
from image_cropper.models import dlib_model_path, face_landmarker_path
from image_cropper.types import CropBox, FaceGeometry

SUPPORTED_EXTENSIONS: set[str] = {".jpg", ".jpeg", ".png", ".webp", ".bmp", ".tiff", ".tif"}

OUTPUT_SIZE: int = 250

# Fallback: extra skull height above the forehead landmark when there is no alpha channel
HEAD_EXTRA_FRACTION: float = 0.18

# How many output pixels (at 250x250) to include below the chin
CHIN_PIXELS_AT_OUTPUT: int = 10

# Minimum pixels of shirt that must be available below the chin to process the image
MIN_SHIRT_PIXELS: int = 20

# Horizontal padding added outside the widest detected face point on each side
EAR_SIDE_PADDING_FRACTION: float = 0.08

# Alpha threshold: pixels with alpha below this are considered empty/background
ALPHA_THRESHOLD: int = 10


def find_hair_top(image: Image.Image) -> int | None:
    """Scan from the top row down and return the first row with any non-transparent pixel.

    Returns None if the image has no alpha channel or is fully transparent.
    """
    if image.mode != "RGBA":
        return None
    alpha = np.array(image)[:, :, 3]
    rows_with_content = np.any(alpha > ALPHA_THRESHOLD, axis=1)
    hits = np.where(rows_with_content)[0]
    return int(hits[0]) if len(hits) else None


def ensure_landmarker() -> bool:
    return face_landmarker_path().exists()


# ---------------------------------------------------------------------------
# Landmark detection
# ---------------------------------------------------------------------------


def _landmarks_mediapipe(image_rgb: np.ndarray) -> list[tuple[float, float]] | None:
    """Return (x, y) pixel coords for all 478 landmarks, or None.

    Index 10 → top of forehead, Index 152 → bottom of chin.
    """
    import mediapipe as mp
    from mediapipe.tasks import python as mp_tasks
    from mediapipe.tasks.python import vision as mp_vision

    h, w = image_rgb.shape[:2]
    base_opts = mp_tasks.BaseOptions(model_asset_path=str(face_landmarker_path()))
    opts = mp_vision.FaceLandmarkerOptions(
        base_options=base_opts,
        output_face_blendshapes=False,
        output_facial_transformation_matrixes=False,
        num_faces=1,
        min_face_detection_confidence=0.3,
        min_face_presence_confidence=0.3,
        min_tracking_confidence=0.3,
    )
    with mp_vision.FaceLandmarker.create_from_options(opts) as detector:
        mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=image_rgb)
        result = detector.detect(mp_image)

    if not result.face_landmarks:
        return None

    lms = result.face_landmarks[0]
    return [(lm.x * w, lm.y * h) for lm in lms]


def _landmarks_dlib(image_rgb: np.ndarray) -> FaceGeometry | None:
    """dlib 68-point landmarks → FaceGeometry. Forehead is estimated from eyes + chin."""
    import cv2
    import dlib

    detector = dlib.get_frontal_face_detector()
    predictor = dlib.shape_predictor(str(dlib_model_path()))

    gray = cv2.cvtColor(image_rgb, cv2.COLOR_RGB2GRAY)
    dets = detector(gray, 1)
    if not dets:
        return None

    det = max(dets, key=lambda d: d.area())
    shape = predictor(gray, det)
    pts = [(shape.part(i).x, shape.part(i).y) for i in range(68)]

    chin_y = pts[8][1]
    eye_y = float(np.mean([pts[i][1] for i in range(36, 48)]))

    face_h = chin_y - eye_y
    forehead_y = eye_y - 0.5 * face_h

    left_x = pts[0][0]
    right_x = pts[16][0]

    return FaceGeometry(
        chin_y=float(chin_y),
        forehead_y=float(forehead_y),
        left_x=float(left_x),
        right_x=float(right_x),
        face_height=float(face_h),
        detector="dlib",
    )


def detect_face_geometry(image_rgb: np.ndarray) -> FaceGeometry | None:
    """Detect face geometry using MediaPipe, falling back to dlib."""
    if face_landmarker_path().exists():
        try:
            lms = _landmarks_mediapipe(image_rgb)
            if lms:
                chin_y = lms[152][1]
                forehead_y = lms[10][1]
                all_x = [p[0] for p in lms]
                left_x = min(all_x)
                right_x = max(all_x)
                return FaceGeometry(
                    chin_y=float(chin_y),
                    forehead_y=float(forehead_y),
                    left_x=float(left_x),
                    right_x=float(right_x),
                    face_height=float(chin_y - forehead_y),
                    detector="MediaPipe",
                )
        except Exception as e:
            print(f"  [!] MediaPipe failed ({e}), trying dlib ...")

    if dlib_model_path().exists():
        try:
            return _landmarks_dlib(image_rgb)
        except Exception as e:
            print(f"  [!] dlib failed ({e})")

    return None


# ---------------------------------------------------------------------------
# Crop computation
# ---------------------------------------------------------------------------


def compute_crop(
    geo: FaceGeometry | dict,
    img_w: int,
    img_h: int,
    hair_top_y: int | None = None,
    chin_pixels: int | None = None,
) -> tuple[int, int, int, int] | None:
    """Compute the portrait crop box.

    Returns ``(left, top, right, bottom)`` or None if there isn't enough shirt
    below the chin. Accepts either a :class:`FaceGeometry` value object or
    the legacy untyped dict for backward compatibility with existing tests.
    """
    if isinstance(geo, FaceGeometry):
        chin_y = geo.chin_y
        forehead_y = geo.forehead_y
        face_h = geo.face_height
        left_x = geo.left_x
        right_x = geo.right_x
    else:
        chin_y = geo["chin_y"]
        forehead_y = geo["forehead_y"]
        face_h = geo["face_height"]
        left_x = geo["left_x"]
        right_x = geo["right_x"]

    assert chin_y > forehead_y, f"chin_y must be below forehead_y: {chin_y} <= {forehead_y}"
    assert right_x > left_x, f"right_x must be > left_x: {right_x} <= {left_x}"

    chin_px = chin_pixels if chin_pixels is not None else CHIN_PIXELS_AT_OUTPUT

    pixels_below_chin = img_h - chin_y
    if pixels_below_chin < MIN_SHIRT_PIXELS:
        return None

    if hair_top_y is not None:
        crop_top = float(hair_top_y)
    else:
        head_extra = face_h * HEAD_EXTRA_FRACTION
        crop_top = max(0.0, forehead_y - head_extra)

    vertical_span = chin_y - crop_top
    chin_margin = vertical_span * (chin_px / OUTPUT_SIZE)
    crop_bottom = min(chin_y + chin_margin, img_h)

    face_center_x = (left_x + right_x) / 2
    half_face_width = (right_x - left_x) / 2
    side_pad = half_face_width * EAR_SIDE_PADDING_FRACTION

    crop_height = crop_bottom - crop_top

    min_half_width = half_face_width + side_pad
    half_w = max(crop_height / 2, min_half_width)

    crop_width = half_w * 2
    crop_height = max(crop_height, crop_width)
    half_w = crop_height / 2

    crop_left = face_center_x - half_w
    crop_right = face_center_x + half_w

    crop_center_y = (crop_top + crop_bottom) / 2
    crop_top = crop_center_y - crop_height / 2
    crop_bottom = crop_center_y + crop_height / 2

    box = (
        int(round(crop_left)),
        int(round(crop_top)),
        int(round(crop_right)),
        int(round(crop_bottom)),
    )

    # Allow ±1px rounding asymmetry — the underlying floats are exactly square,
    # but rounding can shift left/right (or top/bottom) by 1 pixel apart.
    assert abs((box[2] - box[0]) - (box[3] - box[1])) <= 1, (
        f"compute_crop produced non-square box: {box}"
    )
    return box


def crop_with_padding(
    image: Image.Image, left: int, top: int, right: int, bottom: int
) -> Image.Image:
    """Crop, padding with black where the box extends outside the image."""
    img_w, img_h = image.size
    crop_w = right - left
    crop_h = bottom - top

    if left >= 0 and top >= 0 and right <= img_w and bottom <= img_h:
        return image.crop((left, top, right, bottom))

    src_left = max(0, left)
    src_top = max(0, top)
    src_right = min(img_w, right)
    src_bottom = min(img_h, bottom)

    paste_x = src_left - left
    paste_y = src_top - top

    mode = image.mode if image.mode in ("RGB", "RGBA") else "RGB"
    canvas = Image.new(mode, (crop_w, crop_h), color=0)
    patch = image.crop((src_left, src_top, src_right, src_bottom))
    canvas.paste(patch, (paste_x, paste_y))
    return canvas


# ---------------------------------------------------------------------------
# Main processing
# ---------------------------------------------------------------------------


def process_image(input_path: Path, output_path: Path, chin_pixels: int | None = None) -> bool:
    """Crop a portrait image. Raises :class:`DetectionError` on failure."""
    image = Image.open(input_path)
    with contextlib.suppress(Exception):
        from PIL import ImageOps

        image = ImageOps.exif_transpose(image)

    img_rgb = np.array(image.convert("RGB"))
    img_h, img_w = img_rgb.shape[:2]

    hair_top_y = find_hair_top(image)

    geo = detect_face_geometry(img_rgb)
    if geo is None:
        raise DetectionError(f"no face detected in {input_path.name}")

    box = compute_crop(geo, img_w, img_h, hair_top_y=hair_top_y, chin_pixels=chin_pixels)
    if box is None:
        print("  [skip] Not enough shirt below chin — skipping")
        return False

    left, top, right, bottom = box
    crop = CropBox(left=left, top=top, right=right, bottom=bottom)

    hair_src = (
        f"alpha scan row {hair_top_y}" if hair_top_y is not None else "landmark extrapolation"
    )
    print(
        f"  [{geo.detector}]  "
        f"hair_top={crop.top} ({hair_src})  chin={geo.chin_y:.0f}  "
        f"face_h={geo.face_height:.0f}  "
        f"crop=({crop.left},{crop.top})→({crop.right},{crop.bottom})  size={crop.width}"
    )

    cropped = crop_with_padding(image, *crop.as_tuple())
    resized = cropped.resize((OUTPUT_SIZE, OUTPUT_SIZE), Image.LANCZOS)

    output_path.parent.mkdir(parents=True, exist_ok=True)
    resized.save(output_path, "PNG")
    return True


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Crop portraits to 250x250: top-of-head through shirt, face centered."
    )
    parser.add_argument(
        "input",
        nargs="?",
        default="input",
        help="Input image file or directory (default: input/)",
    )
    parser.add_argument(
        "output",
        nargs="?",
        default=None,
        help="Output file or directory (default: output/portrait/ or same dir for single file)",
    )
    parser.add_argument(
        "--chin-pixels",
        type=int,
        default=None,
        help=f"Output pixels below chin (default: {CHIN_PIXELS_AT_OUTPUT})",
    )
    args = parser.parse_args()

    chin_pixels = args.chin_pixels
    input_path = Path(args.input)

    if input_path.is_file():
        if args.output:
            out_path = Path(args.output)
            if out_path.is_dir() or not out_path.suffix:
                out_path = out_path / (input_path.stem + "_portrait.png")
        else:
            out_path = input_path.parent / (input_path.stem + "_portrait.png")

        ensure_landmarker()
        print(f"Processing: {input_path.name}")
        try:
            if process_image(input_path, out_path, chin_pixels=chin_pixels):
                print(f"  Saved → {out_path}")
            else:
                print("  Not saved.")
        except ImageCropperError as e:
            print(f"  [!] {e}")
        except Exception as e:
            print(f"  [!] unexpected error: {e}", file=sys.stderr)
        return

    if not input_path.is_dir():
        print(f"Error: '{input_path}' is not a file or directory.", file=sys.stderr)
        sys.exit(1)

    output_dir = Path(args.output) if args.output else Path("output") / "portrait"
    output_dir.mkdir(parents=True, exist_ok=True)
    ensure_landmarker()

    images = [p for p in sorted(input_path.iterdir()) if p.suffix.lower() in SUPPORTED_EXTENSIONS]
    if not images:
        print(f"No supported images found in {input_path}/")
        sys.exit(0)

    print(f"Found {len(images)} image(s) in {input_path}/\n")
    ok = skipped = errors = 0
    for img_path in images:
        out_path = output_dir / (img_path.stem + ".png")
        print(f"Processing: {img_path.name}")
        try:
            if process_image(img_path, out_path, chin_pixels=chin_pixels):
                ok += 1
                print(f"  Saved → {out_path}\n")
            else:
                skipped += 1
                print()
        except ImageCropperError as e:
            errors += 1
            print(f"  [!] {e}\n")
        except Exception as e:
            errors += 1
            print(f"  [!] unexpected error: {e}\n", file=sys.stderr)

    print(
        f"Done. {ok} saved / {skipped} skipped (no shirt) / {errors} errors  — total {len(images)}"
    )


if __name__ == "__main__":
    main()
