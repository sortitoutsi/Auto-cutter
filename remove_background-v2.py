#!/usr/bin/env python3
"""
High-quality background removal → transparent PNG. (v2)
...
Pipeline (all on GPU when MPS / CUDA is available):
  ...
  7. Alpha power-curve to tighten fringe (reduce white glow)
  8. Alpha decontamination to remove white-background spill from edge pixels
  9. Compose RGBA, save lossless PNG

v2 changes vs v1:
  - pad_to_square fills with white (255,255,255) instead of black to match typical
    white-background inputs and avoid confusing the model at padded borders
  - --refine-radius default raised from 4 → 12 (covers wider halo zone)
  - --refine-eps default lowered from 1e-4 → 1e-6 (sharper edge-locking)
  - Alpha power-curve (alpha^gamma) tightens semi-transparent fringe toward 0
  - Alpha decontamination removes white-background colour spill from edge pixels
...
def pad_to_square(image: Image.Image, fill=(255, 255, 255)) -> tuple[Image.Image, tuple[int, int, int, int]]:
    ...
    Improvement #5: default fill is white to match typical white-background inputs.
    Black padding created an artificial dark border that confused model predictions
    at image edges.
    """
    w, h = image.size
    side = max(w, h)
...
def predict_alpha(
    ...
    padded, _ = pad_to_square(image_rgb)  # white fill (improvement #5)
    acc = None
    count = 0
    for model in models:
...
# ---------------------------------------------------------------------------
# Improvement #2: Alpha power-curve (tighten semi-transparent fringe)
# ---------------------------------------------------------------------------

def alpha_power_curve(alpha: torch.Tensor, gamma: float) -> torch.Tensor:
    """Apply alpha^gamma to push near-0 fringe values toward 0.

    Values already near 1 (solid foreground) are barely affected.
    Values near 0 (background fringe) shrink faster, removing the white glow.
    """
    return alpha.pow(gamma).clamp(0, 1)


# ---------------------------------------------------------------------------
# Improvement #1: Alpha decontamination (remove white-background spill)
# ---------------------------------------------------------------------------

def decontaminate(image_rgb: Image.Image, alpha_hw: torch.Tensor, bg_color: tuple[int, int, int] = (255, 255, 255)) -> Image.Image:
    """Remove background colour spill from semi-transparent edge pixels.

    Edge pixels in the original image are a linear blend of foreground colour
    and background colour: C = alpha * F + (1 - alpha) * B.
    Solving for F: F = (C - (1 - alpha) * B) / alpha.

    Fully opaque pixels (alpha ≈ 1) are untouched. Only the fringe is corrected.
    """
    img_np = np.asarray(image_rgb.convert("RGB"), dtype=np.float32)
    a = alpha_hw.detach().cpu().numpy().astype(np.float32)        # [H, W] in [0,1]
    bg = np.array(bg_color, dtype=np.float32)                     # [3]

    # Broadcast: img_np [H,W,3], a [H,W,1], bg [3]
    a3 = a[..., None]
    fg = (img_np - (1.0 - a3) * bg) / np.clip(a3, 1e-6, 1.0)
    fg = np.clip(fg, 0, 255).astype(np.uint8)
    return Image.fromarray(fg, mode="RGB")
...
def process_image(
    ...
    # Improvement #2: tighten fringe with power curve before decontamination
    if alpha_gamma != 1.0:
        alpha = alpha_power_curve(alpha, alpha_gamma)

    # Improvement #1: remove background colour spill from edge pixels
    if decontam:
...
def main():
    ...
    p.add_argument("--refine-radius", type=int, default=12,
                   help="Guided-filter radius in pixels (default: 12, up from v1's 4)")
    p.add_argument("--refine-eps", type=float, default=1e-6,
                   help="Guided-filter regularisation eps (default: 1e-6, down from v1's 1e-4)")
    p.add_argument("--alpha-gamma", type=float, default=1.3,
                   help="Power-curve exponent applied to alpha (default: 1.3). 1.0 = disabled. "
                        "Higher values clip semi-transparent fringe more aggressively.")
    p.add_argument("--no-decontam", action="store_true", help="Disable background colour decontamination")
    p.add_argument("--bg-color", type=str, default="255,255,255",
                   help="Assumed background colour for decontamination as R,G,B (default: 255,255,255 = white)")
    ...
    try:
        ...
        print(f"Error: --bg-color must be R,G,B integers e.g. 255,255,255", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Settings: tta_flip={not args.no_tta}, ensemble={not args.no_ensemble}, "
          ...
          f"alpha_gamma={args.alpha_gamma}, decontam={not args.no_decontam} (bg={bg_color})\n")
